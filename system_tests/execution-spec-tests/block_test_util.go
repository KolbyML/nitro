...
func (t *BlockTest) Run(snapshotter bool, scheme string, witness bool, tracer *tracing.Hooks, postCheck func(error, *core.BlockChain)) (result error) {
	config, ok := tests.Forks[t.json.Network]
	if !ok {
		return tests.UnsupportedForkError{Name: t.json.Network}
	}

	if slices.Contains([]string{"Prague", "Cancun", "Paris"}, t.json.Network) {
	...
	chain, err := core.NewBlockChain(db, cache, config, gspec, nil, engine, vm.Config{
		Tracer:                  tracer,
